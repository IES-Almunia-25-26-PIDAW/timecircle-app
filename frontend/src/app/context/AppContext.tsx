...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({
              ...
              setUsers((prev) =>
                prev.map((u) => {
                  if (u.id === t.offererId)
                    return {
                      ...u,
                      credits: u.credits + t.creditsAmount,
                      hoursGiven: u.hoursGiven + t.creditsAmount,
                      completedTrades: u.completedTrades + 1,
                    };
                  if (u.id === t.requesterId)
                    return {
                      ...u,
                      credits: Math.max(0, u.credits - t.creditsAmount),
                      hoursReceived: u.hoursReceived + t.creditsAmount,
                      completedTrades: u.completedTrades + 1,
                    };
                  return u;
                }),
              ...
              if (currentUser?.id === t.offererId) {
                setCurrentUser((prev) =>
                  prev
                    ? {
                        ...prev,
                        credits: prev.credits + t.creditsAmount,
                ...
                setCurrentUser((prev) =>
                  prev
                    ? {
                        ...prev,
                        credits: Math.max(0, prev.credits - t.creditsAmount),
                        completedTrades: prev.completedTrades + 1,
                      }
                    : null,
                );
  ...
  const addReview = useCallback(
    (review: Omit<Review, "id" | "createdAt">) => {
      ...
      const userReviews = [...reviews, newReview].filter(
        (r) => r.revieweeId === review.revieweeId,
      );
      const avgRating =
  ...
  const getUserTrades = useCallback(
    (userId: string) =>
      trades.filter((t) => t.offererId === userId || t.requesterId === userId),
    [trades],
  );
