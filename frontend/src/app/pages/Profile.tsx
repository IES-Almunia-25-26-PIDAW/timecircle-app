...
import {
  ...
  Pencil, CheckCircle, Calendar, Tag, Plus, X
} from 'lucide-react';
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl p-6 max-w-lg w-full shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-slate-700 mb-1.5" style={{ fontSize: '0.875rem', fontWeight: 500 }}>Nombre</label>
            <input
              value={name}
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 bg-slate-50 resize-none"
              style={{ fontSize: '0.875rem' }}
            />
          </div>
          <div>
            <label className="block text-slate-700 mb-1.5" style={{ fontSize: '0.875rem', fontWeight: 500 }}>Habilidades</label>
            <div className="flex gap-2 mb-2">
              <input
                value={skillInput}
                onChange={e => setSkillInput(e.target.value)}
                onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addSkill(); } }}
                placeholder="Ej: Cocina, Inglés..."
                ...
                style={{ fontSize: '0.875rem' }}
              />
              <button type="button" onClick={addSkill} className="px-4 py-2 bg-slate-100 rounded-xl hover:bg-slate-200">
                <Plus className="w-4 h-4" />
              </button>
            </div>
            <div className="flex flex-wrap gap-2">
              {skills.map(skill => (
                <span key={skill} className="flex items-center gap-1 px-3 py-1 bg-teal-100 text-teal-700 rounded-full" style={{ fontSize: '0.8rem' }}>
                  {skill}
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={onClose} className="flex-1 py-2.5 border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50" style={{ fontSize: '0.875rem' }}>
            Cancelar
          </button>
          <button onClick={handleSave} className="flex-1 py-2.5 bg-teal-600 text-white rounded-xl hover:bg-teal-700" style={{ fontWeight: 600, fontSize: '0.875rem' }}>
            Guardar
          </button>
        </div>
      </div>
...
export const Profile: React.FC = () => {
  ...
  if (!user) return (
    <div className="text-center py-20 text-slate-400">Usuario no encontrado</div>
  );
          ...
          <div className="px-6 pb-6">
              ...
              <div className="flex gap-2 mt-14">
                {isMe ? (
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 border border-slate-200 text-slate-700 hover:bg-slate-50 rounded-xl transition-colors"
                    style={{ fontSize: '0.875rem' }}
                  >
            ...
            <div className="grid grid-cols-4 gap-4 mt-5 pt-5 border-t border-slate-100">
              {[
                ...
                { label: 'Valoración', val: user.rating > 0 ? `${user.rating}★` : '–', icon: <Star className="w-4 h-4 text-purple-500" /> },
                { label: 'Intercambios', val: user.completedTrades, icon: <ArrowLeftRight className="w-4 h-4 text-teal-500" /> },
                { label: 'Miembro desde', val: new Date(user.memberSince).toLocaleDateString('es-ES', { month: 'short', year: 'numeric' }), icon: <Calendar className="w-4 h-4 text-blue-500" /> },
              ].map(({ label, val, icon }) => (
        ...
        <div className="flex gap-2 border-b border-slate-200">
          {([
            ...
            <button
              ...
              style={{ fontSize: '0.875rem', fontWeight: activeTab === tab ? 600 : 400 }}
            >
              {label}
            </button>
          ))}
                ...
                {userServices.map(service => {
                  ...
                  return (
                    <Link
                      ...
                      className="flex items-start gap-3 p-4 bg-white border border-slate-100 rounded-2xl hover:shadow-sm hover:border-teal-200 transition-all"
                    >
                ...
                return (
                  <div key={review.id} className="bg-white border border-slate-100 rounded-2xl p-5">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="flex-1">
                        <div style={{ fontWeight: 600, fontSize: '0.875rem' }}>{reviewer?.name}</div>
