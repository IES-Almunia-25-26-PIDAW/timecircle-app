...
describe('ActivityTimerBar', () => {
  ...
  it('renders in-progress trade with countdown and calls end actions', async () => {
    const requestEnd = vi.fn().mockResolvedValue(undefined)
    const confirmEnd = vi.fn().mockResolvedValue(undefined)
    ...
    expect(screen.getByText(/Servicio #12/i)).toBeInTheDocument()
    expect(screen.getByText('Testing timer')).toBeInTheDocument()
  ...
  it('renders accepted trade with start actions and refreshes on event', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Solicitar inicio/i }))
    fireEvent.click(screen.getByRole('button', { name: /Confirmar inicio/i }))

    expect(requestStart).toHaveBeenCalledWith(8)
    expect(confirmStart).toHaveBeenCalledWith(8)
